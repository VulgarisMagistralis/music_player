...
@riverpod
Stream readFiles(Ref ref, Directory musicDirectory) =>
    musicDirectory.list(followLinks: false);

@Riverpod(keepAlive: true, dependencies: [totalSongList])
AsyncValue<Playlist> allSongsPlaylist(Ref ref) =>
    ref.watch(totalSongListProvider).whenData(
          (songs) => Playlist(
              id: 'songs',
              name: 'All Songs',
              songIdList: songs.map((Song song) => song.id).toList()),
        );
...
final AsyncNotifierProvider<AudioSessionManager, AudioSessionState?>
    audioSessionManagerProvider =
    AsyncNotifierProvider<AudioSessionManager, AudioSessionState?>(
        () => AudioSessionManager());
final Provider<PlayerAudioHandler> audioHandlerProvider =
    Provider<PlayerAudioHandler>((ref) => PlayerAudioHandler());
@Riverpod(keepAlive: true)
List<String> getSavedFolderList(Ref ref) =>
    LowLevelFolderDataSource().loadFolders();
    ...
    await for (final FileSystemEntity file
      ...
      if (file is File &&
          (path.endsWith('.mp3') ||
              path.endsWith('.wav') ||
              path.endsWith('.webm'))) {
        songs.add(Song.create(file: file));
      }
    }
  }
  return songs;
