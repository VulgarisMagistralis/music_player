...
class _LoadingState extends State<LoadingPage> {
  ...
  Future<PackageInfo> readPackageInfo() async {
    try {
      NewVersionPlus newVersion = await NewVersionPlus(androidId: 'com.cenkt.music_player');
            ...
            FutureBuilder(
                ...
                builder: (context, snapshot) => snapshot.connectionState == ConnectionState.done && snapshot.hasData
                    ? Text('Version: ${snapshot.data?.version} ${storeVersion != null ? '\n New Version Available: ' + storeVersion! : ''}',
                        textAlign: TextAlign.center, style: TextStyle(fontSize: 15, color: Color.fromARGB(106, 122, 122, 122)))
                    : Padding(padding: EdgeInsets.all(10), child: Text('Reading version information', textAlign: TextAlign.center, style: TextStyle(fontSize: 15, color: Color.fromARGB(106, 122, 122, 122)))))
          ])))));
}
