...
class _SettingsPageState extends ConsumerState<SettingsPage> {
  ...
  void initState() {
    super.initState();
    libraryPathList = LowLevelRepositoryImplementation().loadFolderList();
    ...
    if (result != null && result.isNotEmpty) {
      if (!libraryPathList.contains(result)) {
        LowLevelRepositoryImplementation()
            .saveFolderList(folderList: libraryPathList + [result]);
      }
      setState(() => libraryPathList =
          LowLevelRepositoryImplementation().loadFolderList());
    }
  }
                    ...
                    ExpansionTile(
                        title: Row(children: [
                          ...
                          if (libraryPathList.isNotEmpty)
                            Container(
                                ...
                                child: Text(libraryPathList.length.toString(),
                        ...
                        children: libraryPathList
                            .map((libraryPath) => ListTile(
                                title: AnimatedOverflowText(
                                    text: libraryPath.beautifyFolderPath()),
                                trailing: GestureDetector(
                                    child: const Icon(
                                    ...
                                    onTap: () async {
                                      ...
                                      LowLevelRepositoryImplementation()
                                          .saveFolderList(
                                              folderList: libraryPathList);
                                      setState(() {});
                                    })))
                            .toList()),
                    ColorSelector(
                        provider:
                            basicColorProvider(ThemeKeys.mainBackgroundColor),
                        title: 'Background Color'),
                    ColorSelector(
                        provider:
                            basicColorProvider(ThemeKeys.primaryTextColor),
